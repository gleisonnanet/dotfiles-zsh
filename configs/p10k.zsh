# Generated by Powerlevel10k configuration wizard on 2026-04-09 at 16:03 -03.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    # =========================[ Line #2 ]=========================
    newline
    my_project_icon         # file type icons + dir info
    # =========================[ Line #3 ]=========================
    newline                 # \n
  ...
  # Show file type icons + dir stats on the second prompt line
  function prompt_my_project_icon() {
    local -a icons=()
    local -A seen=()
    local ext nfiles=0 ndirs=0
    # Count dirs
    for d in $PWD/*(N/); do
      (( ndirs++ ))
    done
    # Scan files and collect type icons
    for f in $PWD/*(N.); do
      (( nfiles++ ))
      ext=${f:e:l}
    ...
    [[ -f $PWD/Dockerfile || -f $PWD/docker-compose.yml || -f $PWD/docker-compose.yaml ]] && icons+=($'\uF308')
    [[ -f $PWD/Makefile ]] && icons+=($'\uE779')
    # Build info line: icons + stats
    local parts=()
    (( ${#icons} )) && parts+=("${(j: :)icons}")
    # Dir/file counts
    local stats=""
    (( ndirs ))  && stats+=$'\uF07B'" ${ndirs}"
    (( nfiles )) && { [[ -n $stats ]] && stats+="  "; stats+=$'\uF15C'" ${nfiles}"; }
    [[ -n $stats ]] && parts+=("$stats")
    # Permissions
    local perms=$(command stat -c '%A' "$PWD" 2>/dev/null)
    [[ -n $perms ]] && parts+=($'\uF023'" $perms")
    (( ${#parts} )) || return
    p10k segment -f 7 -b 236 -t "${(j:  │  :)parts}"
  }

  typeset -g POWERLEVEL9K_DIR_HOME_VISUAL_IDENTIFIER_EXPANSION=$'\uF015'
  typeset -g POWERLEVEL9K_DIR_HOME_BACKGROUND=4
  typeset -g POWERLEVEL9K_DIR_DOCS_VISUAL_IDENTIFIER_EXPANSION=$'\U000F0219'
  typeset -g POWERLEVEL9K_DIR_DOCS_BACKGROUND=4
  typeset -g POWERLEVEL9K_DIR_DOWNLOADS_VISUAL_IDENTIFIER_EXPANSION=$'\U000F01DA'
  typeset -g POWERLEVEL9K_DIR_DOWNLOADS_BACKGROUND=4
  typeset -g POWERLEVEL9K_DIR_PICTURES_VISUAL_IDENTIFIER_EXPANSION=$'\U000F02E9'
  typeset -g POWERLEVEL9K_DIR_PICTURES_BACKGROUND=4
  typeset -g POWERLEVEL9K_DIR_MUSIC_VISUAL_IDENTIFIER_EXPANSION=$'\U000F075A'
  typeset -g POWERLEVEL9K_DIR_MUSIC_BACKGROUND=4
  typeset -g POWERLEVEL9K_DIR_VIDEOS_VISUAL_IDENTIFIER_EXPANSION=$'\U000F0567'
  typeset -g POWERLEVEL9K_DIR_VIDEOS_BACKGROUND=4
